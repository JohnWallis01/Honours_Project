...
#define SAMPLE_FREQUENCY 10000 //Hz
#define MEASUREMENT_TIME 30 //Seconds
#define TRANSFER_DEPTH 16384
...
int main() {
    ...
    *(uint32_t*)Integrator_Reset = 0;
    ...
    //await DMA
    dma_s2mm_sync(dma_conf_address, FIFO_Empty, 0); // If this locks up make sure all memory ranges are assigned under Address Editor!
    printf(" -- Done.\n");
    printf("Setup Complete: Running Test Procedure -- ");
    //Rising Edge Enables DUT
    *(uint32_t*)Test_Trigger = 0;
    //multiple transfes may be necessary
    int Transfered = 0;
    while(Transfered < SAMPLE_FREQUENCY*MEASUREMENT_TIME) {
        control_set(dma_conf_address, S2MM_CONTROL_REGISTER, 4);
        // Set Destination
        control_set(dma_conf_address, S2MM_DESTINATION_ADDRESS, 0x0f000000); // Write destination address (note that this somehow matters)
        // Start S2MM mode (rev up the bugatti)
        ...
        control_set(dma_conf_address, S2MM_LENGTH, TRANSFER_DEPTH * TRANSFER_WIDTH);
        // await DMA
        dma_s2mm_sync(dma_conf_address, FIFO_Empty, 1); // If this locks up make sure all memory ranges are assigned under Address Editor!
        memdump(dma_dest_address, TRANSFER_DEPTH * TRANSFER_WIDTH);
        Transfered += TRANSFER_DEPTH;
        usleep(TRANSFER_DEPTH / SAMPLE_FREQUENCY);
    }
}
