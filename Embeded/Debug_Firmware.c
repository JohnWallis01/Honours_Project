...
#define FPGA_Delay 2.0


//PLL Tuning
#define kp_value    0xFFFE0000              //bit vector (treated as unisgned)
#define ki_value    0xFFFFFFF8              //bit vector (treated as unisgned)
#define PLL_Lock_Threshold 65000000
#define Demodulation_Threshold_Value 8000000
...
int main() {
    ...
    *(uint32_t*)Integrator_Reset = 0;     //Pulse the Reset 
    ...
    while(1)
    {
        ...
        if(LockStrength < PLL_Lock_Threshold || init) {
                *(uint32_t*)Integrator_Reset = 1;
                *(uint32_t*)PLL_Guess_Freq = 2*f_tuning;
                usleep(1);
                *(uint32_t*)Integrator_Reset = 0;
                printf("Relocking:\n");
