...
with open("log_drift_diffusion.txt", "r") as file:
    data =file.readlines()
...
for line in data:
    if "PLL Error" in line:
        ...
        if nyquist_mode:
            if -(debug_value_actual+delta) < 0:
                tunings.append([debug_value_actual,-(debug_value_actual+delta)+2**32])
            else:
                tunings.append([debug_value_actual,-(debug_value_actual+delta)]) 
        else:
            tunings.append([debug_value_actual,debug_value_actual+delta])
    if "Debug_Value" in line:
        debug_value = int(line.split(":")[1][:-1])
        debug_value_actual = debug_value
        if debug_value < 0:
            debug_value_actual = (debug_value + 2**32) - 2**31
            nyquist_mode = True
        else:
            nyquist_mode = False
