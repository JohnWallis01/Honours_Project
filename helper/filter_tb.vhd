-- -------------------------------------------------------------
--
-- Module: filter_tb_pkg
-- Generated by MATLAB(R) 9.13 and Filter Design HDL Coder 3.1.12.
-- Generated on: 2023-03-21 12:26:46
-- -------------------------------------------------------------

-- -------------------------------------------------------------
-- HDL Code Generation Options:
--
-- TargetLanguage: VHDL
-- TargetDirectory: C:\Users\John\Desktop\Honours_Project\helper
-- AddPipelineRegisters: on
-- Name: CIC32
-- TestBenchStimulus: step ramp chirp noise 
--
-- Filter Settings:
--
-- Discrete-Time FIR Multirate Filter (real)
-- -----------------------------------------
-- Filter Structure        : Cascaded Integrator-Comb Decimator
-- Decimation Factor       : 32
-- Differential Delay      : 1
-- Number of Sections      : 2
-- Stable                  : Yes
-- Linear Phase            : Yes (Type 1)
--
-- Input                   : s16,15
-- Output                  : s26,15
-- Filter Internals        : Full Precision
--   Integrator Section 1  : s26,15
--   Integrator Section 2  : s26,15
--   Comb Section 1        : s26,15
--   Comb Section 2        : s26,15
-- -------------------------------------------------------------
LIBRARY IEEE;
USE IEEE.std_logic_1164.all;
USE IEEE.numeric_std.ALL;


PACKAGE filter_tb_pkg IS

  -- Type Definitions
  TYPE filter_in_data_log_type IS ARRAY (0 TO 3423) OF std_logic_vector(15 DOWNTO 0);
  TYPE filter_out_type IS ARRAY (0 TO 106) OF std_logic_vector(25 DOWNTO 0);

  -- Functions
  FUNCTION to_integer( x : IN std_logic) RETURN integer;
  FUNCTION to_hex( x : IN std_logic) RETURN string;
  FUNCTION to_hex( x : IN std_logic_vector) RETURN string;
  FUNCTION to_hex( x : IN signed ) RETURN string;
  FUNCTION to_hex( x : IN unsigned ) RETURN string;
  FUNCTION to_hex( x : IN real ) RETURN string;
  FUNCTION SLICE( x : IN bit_vector; slice : In Integer) RETURN std_logic_vector;
  FUNCTION SLICE( x : IN bit_vector; slice : In Integer) RETURN signed;
  FUNCTION SLICE( x : IN bit_vector; slice : In Integer) RETURN unsigned;

  -- Procedures
  PROCEDURE filter_in_data_log_procedure 
    (SIGNAL clk      : IN    std_logic;
     SIGNAL reset    : IN    std_logic;
     SIGNAL rdenb    : IN    std_logic;
     SIGNAL addr     : INOUT unsigned(11 DOWNTO 0);
     SIGNAL done     : OUT   std_logic);

  PROCEDURE filter_out_procedure 
    (SIGNAL clk      : IN    std_logic;
     SIGNAL reset    : IN    std_logic;
     SIGNAL rdenb    : IN    std_logic;
     SIGNAL addr     : INOUT unsigned(6 DOWNTO 0);
     SIGNAL done     : OUT   std_logic);

END filter_tb_pkg;

PACKAGE BODY filter_tb_pkg IS
  FUNCTION to_integer( x : IN std_logic) RETURN integer IS
    VARIABLE int: integer;
  BEGIN
    IF x = '0' THEN
      int := 0;
    ELSE
      int := 1;
    END IF;
    RETURN int;
  END;

  FUNCTION to_hex( x : IN std_logic_vector) RETURN string IS
    VARIABLE result  : STRING(1 TO 256); -- 1024 bits max
    VARIABLE i       : INTEGER;
    VARIABLE imod    : INTEGER;
    VARIABLE j       : INTEGER;
    VARIABLE jinc    : INTEGER;
    VARIABLE newx    : std_logic_vector(1023 DOWNTO 0);
  BEGIN
    newx := (OTHERS => '0');
    IF x'LEFT > x'RIGHT THEN
      j := x'LENGTH-1;
      jinc := -1;
    ELSE
      j := 0;
      jinc := 1;
    END IF;
    FOR i IN x'RANGE LOOP
      newx(j) := x(i);
      j := j+jinc;
    END LOOP;  -- i
    i := x'LENGTH-1;
    imod := x'LENGTH MOD 4;
    IF    imod = 1 THEN i := i+3;
    ELSIF imod = 2 THEN i := i+2;
    ELSIF imod = 3 THEN i := i+1;
    END IF;
    j := 1;
    WHILE i >= 3 LOOP
      IF    newx(i DOWNTO (i-3)) = "0000" THEN result(j) := '0';
      ELSIF newx(i DOWNTO (i-3)) = "0001" THEN result(j) := '1';
      ELSIF newx(i DOWNTO (i-3)) = "0010" THEN result(j) := '2';
      ELSIF newx(i DOWNTO (i-3)) = "0011" THEN result(j) := '3';
      ELSIF newx(i DOWNTO (i-3)) = "0100" THEN result(j) := '4';
      ELSIF newx(i DOWNTO (i-3)) = "0101" THEN result(j) := '5';
      ELSIF newx(i DOWNTO (i-3)) = "0110" THEN result(j) := '6';
      ELSIF newx(i DOWNTO (i-3)) = "0111" THEN result(j) := '7';
      ELSIF newx(i DOWNTO (i-3)) = "1000" THEN result(j) := '8';
      ELSIF newx(i DOWNTO (i-3)) = "1001" THEN result(j) := '9';
      ELSIF newx(i DOWNTO (i-3)) = "1010" THEN result(j) := 'A';
      ELSIF newx(i DOWNTO (i-3)) = "1011" THEN result(j) := 'B';
      ELSIF newx(i DOWNTO (i-3)) = "1100" THEN result(j) := 'C';
      ELSIF newx(i DOWNTO (i-3)) = "1101" THEN result(j) := 'D';
      ELSIF newx(i DOWNTO (i-3)) = "1110" THEN result(j) := 'E';
      ELSIF newx(i DOWNTO (i-3)) = "1111" THEN result(j) := 'F';
      ELSE result(j) := 'X';
      END IF;
      i := i-4;
      j := j+1;
    END LOOP;
    RETURN result(1 TO j-1);
  END;


  FUNCTION to_hex( x : IN std_logic ) RETURN string IS
  BEGIN
    RETURN std_logic'image(x);
  END;


  FUNCTION to_hex( x : IN signed ) RETURN string IS
  BEGIN
    RETURN to_hex( std_logic_vector(x) );
  END;


  FUNCTION to_hex( x : IN unsigned ) RETURN string IS
  BEGIN
    RETURN to_hex( std_logic_vector(x) );
  END;


  FUNCTION to_hex( x : IN real ) RETURN string IS
  BEGIN
    RETURN real'image(x);
  END;


  FUNCTION SLICE( x : IN bit_vector; slice : IN Integer) RETURN std_logic_vector IS
    variable result : std_logic_vector(slice - 1 DOWNTO 0);
  BEGIN
    result := to_stdlogicvector(bit_vector'(x))(slice - 1 DOWNTO 0);
    RETURN result;
  END;


  FUNCTION SLICE( x : IN bit_vector; slice : IN Integer) RETURN signed IS
    variable result : signed(slice -  1 DOWNTO 0);
  BEGIN
    result := signed(to_stdlogicvector(bit_vector'(x))(slice - 1 DOWNTO 0));
    RETURN result;
  END;


  FUNCTION SLICE( x : IN bit_vector; slice : IN Integer) RETURN unsigned IS
    variable result : unsigned(slice -  1 DOWNTO 0);
  BEGIN
    result := unsigned(to_stdlogicvector(bit_vector'(x))(slice - 1 DOWNTO 0));
    RETURN result;
  END;


  PROCEDURE filter_in_data_log_procedure 
    (SIGNAL clk      : IN    std_logic;
     SIGNAL reset    : IN    std_logic;
     SIGNAL rdenb    : IN    std_logic;
     SIGNAL addr     : INOUT unsigned(11 DOWNTO 0);
     SIGNAL done     : OUT   std_logic) IS
  BEGIN
-- Counter to generate Addr.
    IF reset = '1' THEN
      addr     <= TO_UNSIGNED(0,12);
    ELSIF clk'event and clk = '1' THEN
      IF rdenb = '1' THEN
        IF (addr = TO_UNSIGNED(3423, 12 )) THEN
          addr     <= addr; 
        ELSE
          addr     <= addr + TO_UNSIGNED(1,12); 
        END IF;
      ELSE 
        addr <= addr;
      END IF;
    END IF;

-- Done Signal generation.
    IF reset = '1' THEN
      done <= '0'; 
    ELSIF (addr = TO_UNSIGNED(3423, 12 )) THEN
      done <= '1'; 
    ELSE
      done <= '0'; 
    END IF;
  END filter_in_data_log_procedure;

  PROCEDURE filter_out_procedure 
    (SIGNAL clk      : IN    std_logic;
     SIGNAL reset    : IN    std_logic;
     SIGNAL rdenb    : IN    std_logic;
     SIGNAL addr     : INOUT unsigned(6 DOWNTO 0);
     SIGNAL done     : OUT   std_logic) IS
  BEGIN
-- Counter to generate Addr.
    IF reset = '1' THEN
      addr     <= TO_UNSIGNED(0,7);
    ELSIF clk'event and clk = '1' THEN
      IF rdenb = '1' THEN
        IF (addr = TO_UNSIGNED(106, 7 )) THEN
          addr     <= addr; 
        ELSE
          addr     <= addr + TO_UNSIGNED(1,7); 
        END IF;
      ELSE 
        addr <= addr;
      END IF;
    END IF;

-- Done Signal generation.
    IF reset = '1' THEN
      done <= '0'; 
    ELSIF (addr = TO_UNSIGNED(106, 7 )) THEN
      done <= '1'; 
    ELSE
      done <= '0'; 
    END IF;
  END filter_out_procedure;

END filter_tb_pkg;

-- -------------------------------------------------------------
--
-- Module: filter_tb_data
-- Generated by MATLAB(R) 9.13 and Filter Design HDL Coder 3.1.12.
-- Generated on: 2023-03-21 12:26:46
-- -------------------------------------------------------------



LIBRARY IEEE;
USE IEEE.std_logic_1164.all;
USE IEEE.numeric_std.ALL;
USE work.filter_tb_pkg.ALL;

PACKAGE filter_tb_data IS

  CONSTANT filter_in_data_log_force : filter_in_data_log_type;
  CONSTANT filter_out_expected : filter_out_type;

END filter_tb_data;

PACKAGE BODY filter_tb_data IS

  CONSTANT filter_in_data_log_force : filter_in_data_log_type :=
    (
         X"0000",
         X"7fff",
         X"7fff",
         X"7fff",
         X"7fff",
         X"7fff",
         X"7fff",
         X"7fff",
         X"7fff",
         X"7fff",
         X"7fff",
         X"7fff",
         X"7fff",
         X"7fff",
         X"7fff",
         X"7fff",
         X"7fff",
         X"7fff",
         X"7fff",
         X"7fff",
         X"7fff",
         X"7fff",
         X"7fff",
         X"7fff",
         X"7fff",
         X"7fff",
         X"7fff",
         X"7fff",
         X"7fff",
         X"7fff",
         X"7fff",
         X"7fff",
         X"7fff",
         X"7fff",
         X"7fff",
         X"7fff",
         X"7fff",
         X"7fff",
         X"7fff",
         X"7fff",
         X"7fff",
         X"7fff",
         X"7fff",
         X"7fff",
         X"7fff",
         X"7fff",
         X"7fff",
         X"7fff",
         X"7fff",
         X"7fff",
         X"7fff",
         X"7fff",
         X"7fff",
         X"7fff",
         X"7fff",
         X"7fff",
         X"7fff",
         X"7fff",
         X"7fff",
         X"7fff",
         X"7fff",
         X"7fff",
         X"7fff",
         X"7fff",
         X"7fff",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"8000",
         X"8040",
         X"8080",
         X"80c0",
         X"8100",
         X"8140",
         X"8180",
         X"81c0",
         X"8201",
         X"8241",
         X"8281",
         X"82c1",
         X"8301",
         X"8341",
         X"8381",
         X"83c1",
         X"8401",
         X"8441",
         X"8481",
         X"84c1",
         X"8501",
         X"8541",
         X"8581",
         X"85c1",
         X"8602",
         X"8642",
         X"8682",
         X"86c2",
         X"8702",
         X"8742",
         X"8782",
         X"87c2",
         X"8802",
         X"8842",
         X"8882",
         X"88c2",
         X"8902",
         X"8942",
         X"8982",
         X"89c2",
         X"8a03",
         X"8a43",
         X"8a83",
         X"8ac3",
         X"8b03",
         X"8b43",
         X"8b83",
         X"8bc3",
         X"8c03",
         X"8c43",
         X"8c83",
         X"8cc3",
         X"8d03",
         X"8d43",
         X"8d83",
         X"8dc3",
         X"8e04",
         X"8e44",
         X"8e84",
         X"8ec4",
         X"8f04",
         X"8f44",
         X"8f84",
         X"8fc4",
         X"9004",
         X"9044",
         X"9084",
         X"90c4",
         X"9104",
         X"9144",
         X"9184",
         X"91c4",
         X"9205",
         X"9245",
         X"9285",
         X"92c5",
         X"9305",
         X"9345",
         X"9385",
         X"93c5",
         X"9405",
         X"9445",
         X"9485",
         X"94c5",
         X"9505",
         X"9545",
         X"9585",
         X"95c5",
         X"9606",
         X"9646",
         X"9686",
         X"96c6",
         X"9706",
         X"9746",
         X"9786",
         X"97c6",
         X"9806",
         X"9846",
         X"9886",
         X"98c6",
         X"9906",
         X"9946",
         X"9986",
         X"99c6",
         X"9a07",
         X"9a47",
         X"9a87",
         X"9ac7",
         X"9b07",
         X"9b47",
         X"9b87",
         X"9bc7",
         X"9c07",
         X"9c47",
         X"9c87",
         X"9cc7",
         X"9d07",
         X"9d47",
         X"9d87",
         X"9dc7",
         X"9e08",
         X"9e48",
         X"9e88",
         X"9ec8",
         X"9f08",
         X"9f48",
         X"9f88",
         X"9fc8",
         X"a008",
         X"a048",
         X"a088",
         X"a0c8",
         X"a108",
         X"a148",
         X"a188",
         X"a1c8",
         X"a209",
         X"a249",
         X"a289",
         X"a2c9",
         X"a309",
         X"a349",
         X"a389",
         X"a3c9",
         X"a409",
         X"a449",
         X"a489",
         X"a4c9",
         X"a509",
         X"a549",
         X"a589",
         X"a5c9",
         X"a60a",
         X"a64a",
         X"a68a",
         X"a6ca",
         X"a70a",
         X"a74a",
         X"a78a",
         X"a7ca",
         X"a80a",
         X"a84a",
         X"a88a",
         X"a8ca",
         X"a90a",
         X"a94a",
         X"a98a",
         X"a9ca",
         X"aa0b",
         X"aa4b",
         X"aa8b",
         X"aacb",
         X"ab0b",
         X"ab4b",
         X"ab8b",
         X"abcb",
         X"ac0b",
         X"ac4b",
         X"ac8b",
         X"accb",
         X"ad0b",
         X"ad4b",
         X"ad8b",
         X"adcb",
         X"ae0c",
         X"ae4c",
         X"ae8c",
         X"aecc",
         X"af0c",
         X"af4c",
         X"af8c",
         X"afcc",
         X"b00c",
         X"b04c",
         X"b08c",
         X"b0cc",
         X"b10c",
         X"b14c",
         X"b18c",
         X"b1cc",
         X"b20d",
         X"b24d",
         X"b28d",
         X"b2cd",
         X"b30d",
         X"b34d",
         X"b38d",
         X"b3cd",
         X"b40d",
         X"b44d",
         X"b48d",
         X"b4cd",
         X"b50d",
         X"b54d",
         X"b58d",
         X"b5cd",
         X"b60e",
         X"b64e",
         X"b68e",
         X"b6ce",
         X"b70e",
         X"b74e",
         X"b78e",
         X"b7ce",
         X"b80e",
         X"b84e",
         X"b88e",
         X"b8ce",
         X"b90e",
         X"b94e",
         X"b98e",
         X"b9ce",
         X"ba0f",
         X"ba4f",
         X"ba8f",
         X"bacf",
         X"bb0f",
         X"bb4f",
         X"bb8f",
         X"bbcf",
         X"bc0f",
         X"bc4f",
         X"bc8f",
         X"bccf",
         X"bd0f",
         X"bd4f",
         X"bd8f",
         X"bdcf",
         X"be10",
         X"be50",
         X"be90",
         X"bed0",
         X"bf10",
         X"bf50",
         X"bf90",
         X"bfd0",
         X"c010",
         X"c050",
         X"c090",
         X"c0d0",
         X"c110",
         X"c150",
         X"c190",
         X"c1d0",
         X"c211",
         X"c251",
         X"c291",
         X"c2d1",
         X"c311",
         X"c351",
         X"c391",
         X"c3d1",
         X"c411",
         X"c451",
         X"c491",
         X"c4d1",
         X"c511",
         X"c551",
         X"c591",
         X"c5d1",
         X"c612",
         X"c652",
         X"c692",
         X"c6d2",
         X"c712",
         X"c752",
         X"c792",
         X"c7d2",
         X"c812",
         X"c852",
         X"c892",
         X"c8d2",
         X"c912",
         X"c952",
         X"c992",
         X"c9d2",
         X"ca13",
         X"ca53",
         X"ca93",
         X"cad3",
         X"cb13",
         X"cb53",
         X"cb93",
         X"cbd3",
         X"cc13",
         X"cc53",
         X"cc93",
         X"ccd3",
         X"cd13",
         X"cd53",
         X"cd93",
         X"cdd3",
         X"ce14",
         X"ce54",
         X"ce94",
         X"ced4",
         X"cf14",
         X"cf54",
         X"cf94",
         X"cfd4",
         X"d014",
         X"d054",
         X"d094",
         X"d0d4",
         X"d114",
         X"d154",
         X"d194",
         X"d1d4",
         X"d215",
         X"d255",
         X"d295",
         X"d2d5",
         X"d315",
         X"d355",
         X"d395",
         X"d3d5",
         X"d415",
         X"d455",
         X"d495",
         X"d4d5",
         X"d515",
         X"d555",
         X"d595",
         X"d5d5",
         X"d616",
         X"d656",
         X"d696",
         X"d6d6",
         X"d716",
         X"d756",
         X"d796",
         X"d7d6",
         X"d816",
         X"d856",
         X"d896",
         X"d8d6",
         X"d916",
         X"d956",
         X"d996",
         X"d9d6",
         X"da17",
         X"da57",
         X"da97",
         X"dad7",
         X"db17",
         X"db57",
         X"db97",
         X"dbd7",
         X"dc17",
         X"dc57",
         X"dc97",
         X"dcd7",
         X"dd17",
         X"dd57",
         X"dd97",
         X"ddd7",
         X"de18",
         X"de58",
         X"de98",
         X"ded8",
         X"df18",
         X"df58",
         X"df98",
         X"dfd8",
         X"e018",
         X"e058",
         X"e098",
         X"e0d8",
         X"e118",
         X"e158",
         X"e198",
         X"e1d8",
         X"e219",
         X"e259",
         X"e299",
         X"e2d9",
         X"e319",
         X"e359",
         X"e399",
         X"e3d9",
         X"e419",
         X"e459",
         X"e499",
         X"e4d9",
         X"e519",
         X"e559",
         X"e599",
         X"e5d9",
         X"e61a",
         X"e65a",
         X"e69a",
         X"e6da",
         X"e71a",
         X"e75a",
         X"e79a",
         X"e7da",
         X"e81a",
         X"e85a",
         X"e89a",
         X"e8da",
         X"e91a",
         X"e95a",
         X"e99a",
         X"e9da",
         X"ea1b",
         X"ea5b",
         X"ea9b",
         X"eadb",
         X"eb1b",
         X"eb5b",
         X"eb9b",
         X"ebdb",
         X"ec1b",
         X"ec5b",
         X"ec9b",
         X"ecdb",
         X"ed1b",
         X"ed5b",
         X"ed9b",
         X"eddb",
         X"ee1c",
         X"ee5c",
         X"ee9c",
         X"eedc",
         X"ef1c",
         X"ef5c",
         X"ef9c",
         X"efdc",
         X"f01c",
         X"f05c",
         X"f09c",
         X"f0dc",
         X"f11c",
         X"f15c",
         X"f19c",
         X"f1dc",
         X"f21d",
         X"f25d",
         X"f29d",
         X"f2dd",
         X"f31d",
         X"f35d",
         X"f39d",
         X"f3dd",
         X"f41d",
         X"f45d",
         X"f49d",
         X"f4dd",
         X"f51d",
         X"f55d",
         X"f59d",
         X"f5dd",
         X"f61e",
         X"f65e",
         X"f69e",
         X"f6de",
         X"f71e",
         X"f75e",
         X"f79e",
         X"f7de",
         X"f81e",
         X"f85e",
         X"f89e",
         X"f8de",
         X"f91e",
         X"f95e",
         X"f99e",
         X"f9de",
         X"fa1f",
         X"fa5f",
         X"fa9f",
         X"fadf",
         X"fb1f",
         X"fb5f",
         X"fb9f",
         X"fbdf",
         X"fc1f",
         X"fc5f",
         X"fc9f",
         X"fcdf",
         X"fd1f",
         X"fd5f",
         X"fd9f",
         X"fddf",
         X"fe20",
         X"fe60",
         X"fea0",
         X"fee0",
         X"ff20",
         X"ff60",
         X"ffa0",
         X"ffe0",
         X"0020",
         X"0060",
         X"00a0",
         X"00e0",
         X"0120",
         X"0160",
         X"01a0",
         X"01e0",
         X"0221",
         X"0261",
         X"02a1",
         X"02e1",
         X"0321",
         X"0361",
         X"03a1",
         X"03e1",
         X"0421",
         X"0461",
         X"04a1",
         X"04e1",
         X"0521",
         X"0561",
         X"05a1",
         X"05e1",
         X"0622",
         X"0662",
         X"06a2",
         X"06e2",
         X"0722",
         X"0762",
         X"07a2",
         X"07e2",
         X"0822",
         X"0862",
         X"08a2",
         X"08e2",
         X"0922",
         X"0962",
         X"09a2",
         X"09e2",
         X"0a23",
         X"0a63",
         X"0aa3",
         X"0ae3",
         X"0b23",
         X"0b63",
         X"0ba3",
         X"0be3",
         X"0c23",
         X"0c63",
         X"0ca3",
         X"0ce3",
         X"0d23",
         X"0d63",
         X"0da3",
         X"0de3",
         X"0e24",
         X"0e64",
         X"0ea4",
         X"0ee4",
         X"0f24",
         X"0f64",
         X"0fa4",
         X"0fe4",
         X"1024",
         X"1064",
         X"10a4",
         X"10e4",
         X"1124",
         X"1164",
         X"11a4",
         X"11e4",
         X"1225",
         X"1265",
         X"12a5",
         X"12e5",
         X"1325",
         X"1365",
         X"13a5",
         X"13e5",
         X"1425",
         X"1465",
         X"14a5",
         X"14e5",
         X"1525",
         X"1565",
         X"15a5",
         X"15e5",
         X"1626",
         X"1666",
         X"16a6",
         X"16e6",
         X"1726",
         X"1766",
         X"17a6",
         X"17e6",
         X"1826",
         X"1866",
         X"18a6",
         X"18e6",
         X"1926",
         X"1966",
         X"19a6",
         X"19e6",
         X"1a27",
         X"1a67",
         X"1aa7",
         X"1ae7",
         X"1b27",
         X"1b67",
         X"1ba7",
         X"1be7",
         X"1c27",
         X"1c67",
         X"1ca7",
         X"1ce7",
         X"1d27",
         X"1d67",
         X"1da7",
         X"1de7",
         X"1e28",
         X"1e68",
         X"1ea8",
         X"1ee8",
         X"1f28",
         X"1f68",
         X"1fa8",
         X"1fe8",
         X"2028",
         X"2068",
         X"20a8",
         X"20e8",
         X"2128",
         X"2168",
         X"21a8",
         X"21e8",
         X"2229",
         X"2269",
         X"22a9",
         X"22e9",
         X"2329",
         X"2369",
         X"23a9",
         X"23e9",
         X"2429",
         X"2469",
         X"24a9",
         X"24e9",
         X"2529",
         X"2569",
         X"25a9",
         X"25e9",
         X"262a",
         X"266a",
         X"26aa",
         X"26ea",
         X"272a",
         X"276a",
         X"27aa",
         X"27ea",
         X"282a",
         X"286a",
         X"28aa",
         X"28ea",
         X"292a",
         X"296a",
         X"29aa",
         X"29ea",
         X"2a2b",
         X"2a6b",
         X"2aab",
         X"2aeb",
         X"2b2b",
         X"2b6b",
         X"2bab",
         X"2beb",
         X"2c2b",
         X"2c6b",
         X"2cab",
         X"2ceb",
         X"2d2b",
         X"2d6b",
         X"2dab",
         X"2deb",
         X"2e2c",
         X"2e6c",
         X"2eac",
         X"2eec",
         X"2f2c",
         X"2f6c",
         X"2fac",
         X"2fec",
         X"302c",
         X"306c",
         X"30ac",
         X"30ec",
         X"312c",
         X"316c",
         X"31ac",
         X"31ec",
         X"322d",
         X"326d",
         X"32ad",
         X"32ed",
         X"332d",
         X"336d",
         X"33ad",
         X"33ed",
         X"342d",
         X"346d",
         X"34ad",
         X"34ed",
         X"352d",
         X"356d",
         X"35ad",
         X"35ed",
         X"362e",
         X"366e",
         X"36ae",
         X"36ee",
         X"372e",
         X"376e",
         X"37ae",
         X"37ee",
         X"382e",
         X"386e",
         X"38ae",
         X"38ee",
         X"392e",
         X"396e",
         X"39ae",
         X"39ee",
         X"3a2f",
         X"3a6f",
         X"3aaf",
         X"3aef",
         X"3b2f",
         X"3b6f",
         X"3baf",
         X"3bef",
         X"3c2f",
         X"3c6f",
         X"3caf",
         X"3cef",
         X"3d2f",
         X"3d6f",
         X"3daf",
         X"3def",
         X"3e30",
         X"3e70",
         X"3eb0",
         X"3ef0",
         X"3f30",
         X"3f70",
         X"3fb0",
         X"3ff0",
         X"4030",
         X"4070",
         X"40b0",
         X"40f0",
         X"4130",
         X"4170",
         X"41b0",
         X"41f0",
         X"4231",
         X"4271",
         X"42b1",
         X"42f1",
         X"4331",
         X"4371",
         X"43b1",
         X"43f1",
         X"4431",
         X"4471",
         X"44b1",
         X"44f1",
         X"4531",
         X"4571",
         X"45b1",
         X"45f1",
         X"4632",
         X"4672",
         X"46b2",
         X"46f2",
         X"4732",
         X"4772",
         X"47b2",
         X"47f2",
         X"4832",
         X"4872",
         X"48b2",
         X"48f2",
         X"4932",
         X"4972",
         X"49b2",
         X"49f2",
         X"4a33",
         X"4a73",
         X"4ab3",
         X"4af3",
         X"4b33",
         X"4b73",
         X"4bb3",
         X"4bf3",
         X"4c33",
         X"4c73",
         X"4cb3",
         X"4cf3",
         X"4d33",
         X"4d73",
         X"4db3",
         X"4df3",
         X"4e34",
         X"4e74",
         X"4eb4",
         X"4ef4",
         X"4f34",
         X"4f74",
         X"4fb4",
         X"4ff4",
         X"5034",
         X"5074",
         X"50b4",
         X"50f4",
         X"5134",
         X"5174",
         X"51b4",
         X"51f4",
         X"5235",
         X"5275",
         X"52b5",
         X"52f5",
         X"5335",
         X"5375",
         X"53b5",
         X"53f5",
         X"5435",
         X"5475",
         X"54b5",
         X"54f5",
         X"5535",
         X"5575",
         X"55b5",
         X"55f5",
         X"5636",
         X"5676",
         X"56b6",
         X"56f6",
         X"5736",
         X"5776",
         X"57b6",
         X"57f6",
         X"5836",
         X"5876",
         X"58b6",
         X"58f6",
         X"5936",
         X"5976",
         X"59b6",
         X"59f6",
         X"5a37",
         X"5a77",
         X"5ab7",
         X"5af7",
         X"5b37",
         X"5b77",
         X"5bb7",
         X"5bf7",
         X"5c37",
         X"5c77",
         X"5cb7",
         X"5cf7",
         X"5d37",
         X"5d77",
         X"5db7",
         X"5df7",
         X"5e38",
         X"5e78",
         X"5eb8",
         X"5ef8",
         X"5f38",
         X"5f78",
         X"5fb8",
         X"5ff8",
         X"6038",
         X"6078",
         X"60b8",
         X"60f8",
         X"6138",
         X"6178",
         X"61b8",
         X"61f8",
         X"6239",
         X"6279",
         X"62b9",
         X"62f9",
         X"6339",
         X"6379",
         X"63b9",
         X"63f9",
         X"6439",
         X"6479",
         X"64b9",
         X"64f9",
         X"6539",
         X"6579",
         X"65b9",
         X"65f9",
         X"663a",
         X"667a",
         X"66ba",
         X"66fa",
         X"673a",
         X"677a",
         X"67ba",
         X"67fa",
         X"683a",
         X"687a",
         X"68ba",
         X"68fa",
         X"693a",
         X"697a",
         X"69ba",
         X"69fa",
         X"6a3b",
         X"6a7b",
         X"6abb",
         X"6afb",
         X"6b3b",
         X"6b7b",
         X"6bbb",
         X"6bfb",
         X"6c3b",
         X"6c7b",
         X"6cbb",
         X"6cfb",
         X"6d3b",
         X"6d7b",
         X"6dbb",
         X"6dfb",
         X"6e3c",
         X"6e7c",
         X"6ebc",
         X"6efc",
         X"6f3c",
         X"6f7c",
         X"6fbc",
         X"6ffc",
         X"703c",
         X"707c",
         X"70bc",
         X"70fc",
         X"713c",
         X"717c",
         X"71bc",
         X"71fc",
         X"723d",
         X"727d",
         X"72bd",
         X"72fd",
         X"733d",
         X"737d",
         X"73bd",
         X"73fd",
         X"743d",
         X"747d",
         X"74bd",
         X"74fd",
         X"753d",
         X"757d",
         X"75bd",
         X"75fd",
         X"763e",
         X"767e",
         X"76be",
         X"76fe",
         X"773e",
         X"777e",
         X"77be",
         X"77fe",
         X"783e",
         X"787e",
         X"78be",
         X"78fe",
         X"793e",
         X"797e",
         X"79be",
         X"79fe",
         X"7a3f",
         X"7a7f",
         X"7abf",
         X"7aff",
         X"7b3f",
         X"7b7f",
         X"7bbf",
         X"7bff",
         X"7c3f",
         X"7c7f",
         X"7cbf",
         X"7cff",
         X"7d3f",
         X"7d7f",
         X"7dbf",
         X"7dff",
         X"7e40",
         X"7e80",
         X"7ec0",
         X"7f00",
         X"7f40",
         X"7f80",
         X"7fc0",
         X"7fff",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"7fff",
         X"7fff",
         X"7fff",
         X"7ffd",
         X"7ff7",
         X"7fe9",
         X"7fd0",
         X"7fa7",
         X"7f68",
         X"7f0d",
         X"7e8e",
         X"7de2",
         X"7d02",
         X"7be2",
         X"7a79",
         X"78bc",
         X"769f",
         X"7416",
         X"7116",
         X"6d93",
         X"6981",
         X"64d6",
         X"5f87",
         X"598b",
         X"52db",
         X"4b71",
         X"434b",
         X"3a68",
         X"30ca",
         X"2679",
         X"1b7f",
         X"0fec",
         X"03d4",
         X"f751",
         X"ea82",
         X"dd8b",
         X"d098",
         X"c3d9",
         X"b781",
         X"abcc",
         X"a0f5",
         X"973d",
         X"8ee4",
         X"882c",
         X"8353",
         X"8092",
         X"801e",
         X"8220",
         X"86b8",
         X"8df5",
         X"97d9",
         X"a450",
         X"b334",
         X"c445",
         X"d731",
         X"eb8d",
         X"00d7",
         X"167d",
         X"2bda",
         X"403c",
         X"52ed",
         X"6336",
         X"7067",
         X"79df",
         X"7f16",
         X"7fa5",
         X"7b4f",
         X"7205",
         X"63f1",
         X"5174",
         X"3b2d",
         X"21f3",
         X"06d0",
         X"eafc",
         X"cfcb",
         X"b6a3",
         X"a0e5",
         X"8fdb",
         X"84a1",
         X"8012",
         X"82b3",
         X"8ca2",
         X"9d8d",
         X"b4ac",
         X"d0c5",
         X"f038",
         X"1110",
         X"3124",
         X"4e36",
         X"6620",
         X"76fa",
         X"7f4a",
         X"7e28",
         X"7359",
         X"5f67",
         X"43a0",
         X"220b",
         X"fd49",
         X"d863",
         X"b68f",
         X"9ae7",
         X"881b",
         X"802d",
         X"842d",
         X"9413",
         X"aea8",
         X"d193",
         X"f985",
         X"227f",
         X"4837",
         X"6687",
         X"79ed",
         X"7ff5",
         X"7793",
         X"615e",
         X"3f93",
         X"15ed",
         X"e947",
         X"bf10",
         X"9ca4",
         X"8699",
         X"801a",
         X"8a66",
         X"a483",
         X"cb45",
         X"f99d",
         X"2936",
         X"534a",
         X"719f",
         X"7f76",
         X"7a5d",
         X"62ab",
         X"3b9c",
         X"0af3",
         X"d835",
         X"ab83",
         X"8c4e",
         X"8010",
         X"8940",
         X"a6c3",
         X"d3ea",
         X"091c",
         X"3d09",
         X"664b",
         X"7d1b",
         X"7cdd",
         X"6523",
         X"39fc",
         X"035b",
         X"cbba",
         X"9e1d",
         X"83e4",
         X"82c7",
         X"9b7c",
         X"c958",
         X"030c",
         X"3c71",
         X"690f",
         X"7eda",
         X"788b",
         X"570b",
         X"217e",
         X"e3e8",
         X"ac88",
         X"8896",
         X"8107",
         X"982e",
         X"c8e8",
         X"0791",
         X"44ab",
         X"70ac",
         X"7ffa",
         X"6e20",
         X"3f3f",
         X"ff51",
         X"bf3c",
         X"906c",
         X"8006",
         X"9319",
         X"c4db",
         X"07a9",
         X"489a",
         X"74cd",
         X"7ef5",
         X"6387",
         X"2a25",
         X"e3b9",
         X"a59d",
         X"8327",
         X"8789",
         X"b1f6",
         X"f579",
         X"3cb1",
         X"707c",
         X"7f8a",
         X"644f",
         X"2751",
         X"dcba",
         X"9ddf",
         X"80a3",
         X"8fb4",
         X"c665",
         X"11d0",
         X"5729",
         X"7d35",
         X"7596",
         X"427d",
         X"f660",
         X"ad91",
         X"83c7",
         X"8977",
         X"bd0f",
         X"0b04",
         X"54f3",
         X"7d62",
         X"73a5",
         X"3b06",
         X"ea0e",
         X"a1d0",
         X"807a",
         X"9488",
         X"d62b",
         X"29ed",
         X"6bf0",
         X"7f4b",
         X"5aed",
         X"0e39",
         X"bae5",
         X"8655",
         X"88c0",
         X"c1b6",
         X"179a",
         X"62d5",
         X"7ffe",
         X"60c0",
         X"1349",
         X"bc47",
         X"85c6",
         X"8ac6",
         X"c981",
         X"237f",
         X"6c0a",
         X"7e59",
         X"5081",
         X"f944",
         X"a536",
         X"801f",
         X"9df7",
         X"efb7",
         X"4a67",
         X"7d75",
         X"6cd6",
         X"20dc",
         X"c29a",
         X"8602",
         X"8d36",
         X"d4e4",
         X"352d",
         X"7775",
         X"755d",
         X"2f64",
         X"cd99",
         X"8916",
         X"8aa8",
         X"d217",
         X"3543",
         X"7890",
         X"72ba",
         X"2699",
         X"c26f",
         X"843c",
         X"934a",
         X"e6e0",
         X"4aa2",
         X"7f00",
         X"61d2",
         X"0536",
         X"a4f2",
         X"8033",
         X"afd4",
         X"14ef",
         X"6c4a",
         X"7ad7",
         X"3607",
         X"cc1a",
         X"859c",
         X"93b2",
         X"ed52",
         X"5449",
         X"7ffc",
         X"50d0",
         X"e7b4",
         X"8fd3",
         X"8906",
         X"d8fc",
         X"45c8",
         X"7f4c",
         X"5a53",
         X"f1b3",
         X"9396",
         X"875d",
         X"d710",
         X"4658",
         X"7f9b",
         X"55ee",
         X"e908",
         X"8def",
         X"8cd3",
         X"e756",
         X"55cb",
         X"7f6c",
         X"41ef",
         X"ce96",
         X"837d",
         X"9ee4",
         X"0ae5",
         X"6e0b",
         X"750f",
         X"1948",
         X"a7d8",
         X"8172",
         X"c7a9",
         X"3eb0",
         X"7f80",
         X"510b",
         X"db47",
         X"85ba",
         X"9caa",
         X"0c6d",
         X"713e",
         X"6feb",
         X"08e1",
         X"9987",
         X"881f",
         X"e571",
         X"5b9d",
         X"7c6f",
         X"2872",
         X"ae1b",
         X"814b",
         X"cd58",
         X"4a26",
         X"7fa4",
         X"396c",
         X"bb15",
         X"8014",
         X"c308",
         X"4281",
         X"7ff9",
         X"3d6f",
         X"bcf6",
         X"8014",
         X"c529",
         X"467d",
         X"7fa1",
         X"3515",
         X"b354",
         X"8155",
         X"d40c",
         X"5535",
         X"7c58",
         X"1f36",
         X"a08c",
         X"8849",
         X"f14f",
         X"6a71",
         X"6fa4",
         X"fa75",
         X"8b33",
         X"9d15",
         X"1d08",
         X"7cb8",
         X"5075",
         X"c953",
         X"8000",
         X"c86d",
         X"5095",
         X"7c3b",
         X"185b",
         X"980f",
         X"90db",
         X"0bee",
         X"791f",
         X"5727",
         X"cd8b",
         X"8001",
         X"cbeb",
         X"56b3",
         X"78b1",
         X"07f0",
         X"8d58",
         X"9f50",
         X"2883",
         X"7fb5",
         X"3825",
         X"aa56",
         X"87e1",
         X"fcd2",
         X"75ee",
         X"592f",
         X"ca78",
         X"8047",
         X"da9b",
         X"6570",
         X"6c8e",
         X"e5f2",
         X"815a",
         X"c370",
         X"55b2",
         X"7649",
         X"f920",
         X"850b",
         X"b641",
         X"4b25",
         X"7a3e",
         X"02da",
         X"8770",
         X"b187",
         X"47e9",
         X"7ad4",
         X"02f3",
         X"86d6",
         X"b487",
         X"4c8a",
         X"7879",
         X"f96b",
         X"83a2",
         X"bfbb",
         X"583c",
         X"7193",
         X"e66d",
         X"805a",
         X"d490",
         X"6885",
         X"62c8",
         X"cb18",
         X"81c2",
         X"f458",
         X"7872",
         X"47e5",
         X"aaff",
         X"8e89",
         X"1e51",
         X"7fff",
         X"1def",
         X"8dd4",
         X"ada0",
         X"4cea",
         X"74d8",
         X"e69f",
         X"8006",
         X"e264",
         X"7383",
         X"4de9",
         X"ac86",
         X"9022",
         X"26bf",
         X"7f38",
         X"0aa8",
         X"84c9",
         X"c6ed",
         X"6620",
         X"5e5e",
         X"bbb1",
         X"8978",
         X"1b35",
         X"7fe0",
         X"0fa4",
         X"8536",
         X"c8a7",
         X"6948",
         X"582a",
         X"b19a",
         X"9051",
         X"2da6",
         X"7cdd",
         X"f580",
         X"8031",
         X"e808",
         X"798a",
         X"374f",
         X"9458",
         X"ae34",
         X"5814",
         X"666d",
         X"bf3c",
         X"8b2b",
         X"278f",
         X"7d2c",
         X"f1f6",
         X"8002",
         X"f579",
         X"7e12",
         X"2138",
         X"87ae",
         X"ca8f",
         X"6fb2",
         X"46e0",
         X"9ae9",
         X"aa8e",
         X"5952",
         X"6140",
         X"b2ea",
         X"957c",
         X"40f7",
         X"718c",
         X"ca99",
         X"8951",
         X"2abe",
         X"7a48",
         X"dec7",
         X"8351",
         X"1905",
         X"7e31",
         X"edc5",
         X"80ee",
         X"0ced",
         X"7f8a",
         X"f6dd",
         X"803d",
         X"06e0",
         X"7fd8",
         X"f9d9",
         X"8029",
         X"06f8",
         X"7fc1",
         X"f6ad",
         X"807b",
         X"0d35",
         X"7f08",
         X"ed65",
         X"81e1",
         X"197b",
         X"7c91",
         X"de3b",
         X"85e6",
         X"2b5d",
         X"766b",
         X"c9ea",
         X"8ed4",
         X"41b2",
         X"6a04",
         X"b229",
         X"9f65",
         X"5a10",
         X"54a3",
         X"9a38",
         X"ba1c",
         X"704a",
         X"3448",
         X"873c",
         X"e01a",
         X"7e4e",
         X"0912",
         X"8009",
         X"0f96",
         X"7cd4",
         X"d6e0",
         X"8bdf",
         X"4244",
         X"6557",
         X"a699",
         X"afaa",
         X"6cb1",
         X"3584",
         X"85d7",
         X"ea13",
         X"7fe8",
         X"f357",
         X"83a5",
         X"2fc1",
         X"6e8a",
         X"afc0",
         X"a999",
         X"6aa8",
         X"3515",
         X"8484",
         X"f2f0",
         X"7fbb",
         X"e22c",
         X"8a87",
         X"46a6",
         X"5c6e",
         X"982b",
         X"c98f",
         X"7bfd",
         X"07a5",
         X"8128",
         X"29b4",
         X"6e4d",
         X"aa1e",
         X"b4b2",
         X"74e5",
         X"1a57",
         X"8001",
         X"1cbe",
         X"7360",
         X"b049",
         X"b067",
         X"73c5",
         X"1a58",
         X"800e",
         X"21f8",
         X"6fa5",
         X"a825",
         X"bb61",
         X"79ba",
         X"07a8",
         X"827e",
         X"389d",
         X"6017",
         X"951d",
         X"d895",
         X"7fe5",
         X"e231",
         X"9084",
         X"5b64",
         X"3c5b",
         X"82a4",
         X"0b25",
         X"76d5",
         X"afc5",
         X"b863",
         X"7a9d",
         X"fe23",
         X"86a1",
         X"4bea",
         X"4a80",
         X"85da",
         X"0232",
         X"78a3",
         X"b0c1",
         X"ba8e",
         X"7c4f",
         X"f3db",
         X"8bdb",
         X"599d",
         X"37bb",
         X"80ad",
         X"1f6f",
         X"6981",
         X"973d",
         X"e00e",
         X"7f38",
         X"c54c",
         X"ab12",
         X"7814",
         X"fd49",
         X"8a0f",
         X"5a05",
         X"32a8",
         X"8007",
         X"2e47",
         X"5c87",
         X"8ab6",
         X"fe54",
         X"7671",
         X"a4ba",
         X"d1d7",
         X"7fe3",
         X"c7a1",
         X"adc3",
         X"7b18",
         X"ed78",
         X"9454",
         X"6bbc",
         X"11a8",
         X"8598",
         X"55d0",
         X"3140",
         X"8028",
         X"3cf1",
         X"4ad6",
         X"81e0",
         X"23f7",
         X"5e2f",
         X"886b",
         X"0cd6",
         X"6bdc",
         X"91a0",
         X"f8bb",
         X"74db",
         X"9bb0",
         X"e834",
         X"7a50",
         X"a536",
         X"db63",
         X"7d51",
         X"ad34",
         X"d22e",
         X"7ec8",
         X"b2ff",
         X"cc64",
         X"7f62",
         X"b630",
         X"c9d8",
         X"7f88",
         X"b696",
         X"ca75",
         X"7f5d",
         X"b429",
         X"ce3e",
         X"7eba",
         X"af10",
         X"d555",
         X"7d33",
         X"a7a1",
         X"dfec",
         X"7a16",
         X"9e73",
         X"ee2c",
         X"7478",
         X"946a",
         X"001f",
         X"6b3f",
         X"8ace",
         X"157e",
         X"5d49",
         X"8350",
         X"2d85",
         X"499b",
         X"8004",
         X"46bc",
         X"2fae",
         X"8339",
         X"5ed9",
         X"0fc9",
         X"8f2f",
         X"72b0",
         X"eb6b",
         X"a58f",
         X"7e6d",
         X"c59b",
         X"c6ca",
         X"7e15",
         X"a306",
         X"f14f",
         X"6e65",
         X"89ae",
         X"20f4",
         X"4e0a",
         X"8000",
         X"4ec6",
         X"1ef0",
         X"8b44",
         X"71a3",
         X"e742",
         X"ad7f",
         X"7ff8",
         X"b180",
         X"e35a",
         X"7293",
         X"8b02",
         X"22e5",
         X"480a",
         X"80ab",
         X"5c45",
         X"07a8",
         X"9a08",
         X"7d32",
         X"c20e",
         X"d464",
         X"772f",
         X"8e15",
         X"2019",
         X"46ee",
         X"815b",
         X"62cb",
         X"f99c",
         X"a616",
         X"7fed",
         X"ac27",
         X"f287",
         X"6601",
         X"81be",
         X"47bf",
         X"1b05",
         X"931b",
         X"7c13",
         X"bff8",
         X"dda4",
         X"6ff0",
         X"851a",
         X"3d78",
         X"23b5",
         X"9022",
         X"7b5c",
         X"bfb9",
         X"e0e4",
         X"6cac",
         X"82c8",
         X"4837",
         X"1468",
         X"9a6c",
         X"7f5e",
         X"ab83",
         X"fc99",
         X"5935",
         X"8038",
         X"6382",
         X"ec3b",
         X"ba22",
         X"7b91",
         X"8d72",
         X"3010",
         X"2a25",
         X"90c9",
         X"7d89",
         X"b13a",
         X"fa2e",
         X"5754",
         X"80e2",
         X"6af2",
         X"db11",
         X"ce14",
         X"718c",
         X"82bf",
         X"5063",
         X"004a",
         X"af76",
         X"7d0e",
         X"8d27",
         X"3703",
         X"1c2f",
         X"9ceb",
         X"7ff5",
         X"98b7",
         X"23f4",
         X"2dde",
         X"934a",
         X"7f7e",
         X"a0b9",
         X"197b",
         X"3607",
         X"8fca",
         X"7ef3",
         X"a2cf",
         X"1858",
         X"3555",
         X"911c",
         X"7f75",
         X"9e73",
         X"209a",
         X"2bb7",
         X"97c4",
         X"7ffa",
         X"94c8",
         X"31b9",
         X"186f",
         X"a5fa",
         X"7d4d",
         X"88f4",
         X"49ea",
         X"faed",
         X"beed",
         X"7243",
         X"80a2",
         X"64dc",
         X"d485",
         X"e527",
         X"58c2",
         X"8411",
         X"7a6d",
         X"aac2",
         X"17a9",
         X"2c5d",
         X"9c79",
         X"7ea2",
         X"895a",
         X"4e51",
         X"eef4",
         X"cf8d",
         X"64d2",
         X"8166",
         X"7795",
         X"ade6",
         X"1820",
         X"278d",
         X"a2dd",
         X"7bc1",
         X"8390",
         X"5f8a",
         X"d3a2",
         X"ef30",
         X"49c9",
         X"8e46",
         X"7fff",
         X"8e15",
         X"4b11",
         X"ebba",
         X"d97e",
         X"58d8",
         X"878b",
         X"7f38",
         X"93c3",
         X"43ca",
         X"f1bf",
         X"d64a",
         X"5955",
         X"883e",
         X"7fa7",
         X"8ffe",
         X"4c25",
         X"e51d",
         X"e536",
         X"4b71",
         X"912d",
         X"7f38",
         X"85b6",
         X"614c",
         X"c747",
         X"0762",
         X"2ac5",
         X"aa10",
         X"73ae",
         X"802e",
         X"78f4",
         X"9f7e",
         X"3a65",
         X"f3ae",
         X"dce2",
         X"4d63",
         X"9319",
         X"7dbf",
         X"81f1",
         X"6e35",
         X"af6b",
         X"2933",
         X"02e3",
         X"d1bd",
         X"53d2",
         X"90a4",
         X"7dfe",
         X"8197",
         X"70f0",
         X"a899",
         X"34d4",
         X"f2ee",
         X"e456",
         X"4142",
         X"9fe9",
         X"755d",
         X"80ac",
         X"7d61",
         X"8ff6",
         X"58d4",
         X"c5ee",
         X"16a5",
         X"0e55",
         X"ce3e",
         X"50d0",
         X"96d4",
         X"7928",
         X"802f",
         X"7cf5",
         X"8ee9",
         X"5d5a",
         X"bca0",
         X"2524",
         X"fb36",
         X"e481",
         X"39ad",
         X"ac0c",
         X"68e0",
         X"8899",
         X"7eed",
         X"80b8",
         X"78b7",
         X"9429",
         X"5992",
         X"bcf6",
         X"2987",
         X"f19f",
         X"f2f0",
         X"278a",
         X"c016",
         X"553a",
         X"9949",
         X"73d4",
         X"83c2",
         X"7fd8",
         X"8148",
         X"791f",
         X"908b",
         X"6240",
         X"ade6",
         X"3fac",
         X"d45a",
         X"16b4",
         X"fe83",
         X"ec9e",
         X"275f",
         X"c5ff",
         X"4ae2",
         X"a64c",
         X"663c",
         X"8fad",
         X"77e8",
         X"8307",
         X"7f94",
         X"802d",
         X"7dde",
         X"861d",
         X"7417",
         X"934e",
         X"63f1",
         X"a5f0",
         X"4f48",
         X"bc2c",
         X"37e8",
         X"d448",
         X"1f6f",
         X"ecca",
         X"0732",
         X"0481",
         X"f038",
         X"1a8f",
         X"db3a",
         X"2e61",
         X"c8a7",
         X"3fa9",
         X"b8b1",
         X"4e4d",
         X"ab5a",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"5092",
         X"67e2",
         X"a082",
         X"69d3",
         X"21e2",
         X"98f8",
         X"c74c",
         X"0c00",
         X"751f",
         X"7703",
         X"a859",
         X"7879",
         X"7509",
         X"fc42",
         X"4cdf",
         X"a453",
         X"ebf9",
         X"6a6e",
         X"4ace",
         X"75a1",
         X"27df",
         X"8924",
         X"5961",
         X"6f1a",
         X"2dc2",
         X"41fb",
         X"3e3e",
         X"e469",
         X"27cd",
         X"abd3",
         X"34bf",
         X"8826",
         X"c6e4",
         X"8bd2",
         X"98de",
         X"52ce",
         X"31e0",
         X"d12d",
         X"7342",
         X"88d1",
         X"f052",
         X"e1ae",
         X"43f9",
         X"4b92",
         X"afd7",
         X"fd61",
         X"f212",
         X"2575",
         X"3599",
         X"4133",
         X"c6aa",
         X"2e01",
         X"27b5",
         X"a9a1",
         X"9e77",
         X"ff95",
         X"75b2",
         X"d724",
         X"15d4",
         X"b94c",
         X"4053",
         X"c14e",
         X"0186",
         X"32f7",
         X"6412",
         X"7594",
         X"0c16",
         X"a37d",
         X"a638",
         X"c1ec",
         X"5739",
         X"c119",
         X"5075",
         X"be58",
         X"6de4",
         X"d999",
         X"b254",
         X"c047",
         X"1db5",
         X"f929",
         X"da06",
         X"54b1",
         X"15d4",
         X"0cbb",
         X"6acd",
         X"c92d",
         X"41d8",
         X"40f4",
         X"e165",
         X"115d",
         X"936b",
         X"8dd0",
         X"07e2",
         X"4778",
         X"6f1b",
         X"a142",
         X"119e",
         X"f82a",
         X"830c",
         X"d64e",
         X"a985",
         X"4b56",
         X"cfac",
         X"074e",
         X"aa68",
         X"1a1b",
         X"c352",
         X"2772",
         X"3070",
         X"3f87",
         X"f357",
         X"9575",
         X"ba9e",
         X"69d0",
         X"a702",
         X"5369",
         X"09d1",
         X"7f03",
         X"9403",
         X"f153",
         X"9b4e",
         X"763f",
         X"8130",
         X"4661",
         X"513b",
         X"5e63",
         X"959e",
         X"e658",
         X"c287",
         X"4cd1",
         X"ee71",
         X"6920",
         X"ae8e",
         X"c389",
         X"a542",
         X"a2d5",
         X"5e8a",
         X"1468",
         X"0cc4",
         X"a51c",
         X"5a60",
         X"1f3f",
         X"d9d8",
         X"0364",
         X"e6dd",
         X"9373",
         X"bd6b",
         X"9f92",
         X"af15",
         X"bd6e",
         X"ead2",
         X"8cb6",
         X"6718",
         X"71de",
         X"fda9",
         X"fd40",
         X"d675",
         X"666a",
         X"de87",
         X"9c78",
         X"47bf",
         X"e3c6",
         X"bddf",
         X"e767",
         X"98b1",
         X"a1c9",
         X"712a",
         X"74c5",
         X"1341",
         X"8f4e",
         X"bc1b",
         X"da69",
         X"523a",
         X"83f1",
         X"8b04",
         X"ab43",
         X"262c",
         X"3b52",
         X"25d3",
         X"f370",
         X"0c09",
         X"cbdc",
         X"3ea4",
         X"b05f",
         X"2fd1",
         X"aefb",
         X"de55",
         X"2029",
         X"47bd",
         X"94c5",
         X"6dec",
         X"4695",
         X"fc9e",
         X"ef94",
         X"f260",
         X"ce6d",
         X"022e",
         X"02c2",
         X"5150",
         X"4b7a",
         X"24f2",
         X"e0ed",
         X"4fc4",
         X"0867",
         X"d9c9",
         X"7062",
         X"603e",
         X"0cd7",
         X"1f5b",
         X"1649",
         X"b52f",
         X"cd1e",
         X"f88e",
         X"bb01",
         X"5825",
         X"b1dc",
         X"b9d6",
         X"abb4",
         X"ba48",
         X"ef8a",
         X"cfa4",
         X"6c63",
         X"ee22",
         X"af50",
         X"67a6",
         X"7ad1",
         X"f05a",
         X"9c72",
         X"c211",
         X"e8a2",
         X"184b",
         X"c320",
         X"1a54",
         X"3612",
         X"b8c4",
         X"9e0f",
         X"cbf3",
         X"d19b",
         X"ec96",
         X"0203",
         X"95e4",
         X"c332",
         X"4d0f",
         X"877b",
         X"6dc9",
         X"3af7",
         X"fd15",
         X"141a",
         X"bcbf",
         X"f577",
         X"768d",
         X"0bfb",
         X"0569",
         X"bb4a",
         X"fd28",
         X"1fc2",
         X"2ddc",
         X"e540",
         X"de10",
         X"7cec",
         X"89a9",
         X"629a",
         X"69cd",
         X"4bd3",
         X"9945",
         X"c30a",
         X"d5da",
         X"2e03",
         X"a2f5",
         X"38a2",
         X"9b55",
         X"275d",
         X"fe82",
         X"4770",
         X"370d",
         X"675a",
         X"6413",
         X"d58c",
         X"32e1",
         X"b2a4",
         X"87d2",
         X"3e7c",
         X"0001",
         X"fadc",
         X"679c",
         X"1c20",
         X"1e1f",
         X"5c04",
         X"4e35",
         X"13a4",
         X"aed4",
         X"bd6c",
         X"62f2",
         X"8757",
         X"fd6a",
         X"aafd",
         X"7a8b",
         X"3673",
         X"001f",
         X"f899",
         X"8f43",
         X"2e96",
         X"8add",
         X"924a",
         X"058b",
         X"98c3",
         X"5172",
         X"514b",
         X"38f2",
         X"a65e",
         X"28dc",
         X"04c3",
         X"7915",
         X"2624",
         X"4ce2",
         X"f42c",
         X"eeb1",
         X"5348",
         X"955e",
         X"a217",
         X"ac63",
         X"e414",
         X"54d5",
         X"4da9",
         X"8f7b",
         X"e636",
         X"06e1",
         X"eab3",
         X"2828",
         X"20c3",
         X"cabf",
         X"ee81",
         X"83f7",
         X"7bec",
         X"aacc",
         X"9b31",
         X"df56",
         X"b2b8",
         X"fd5c",
         X"d6e9",
         X"739e",
         X"6b9b",
         X"8d7c",
         X"3ce4",
         X"c4e5",
         X"ec3f",
         X"0c41",
         X"7157",
         X"eaf1",
         X"7ba9",
         X"cd2c",
         X"337b",
         X"2a95",
         X"0a04",
         X"32b7",
         X"2aa2",
         X"ad9a",
         X"a0c6",
         X"7fc4",
         X"abcf",
         X"8859",
         X"0fab",
         X"61c2",
         X"2b4f",
         X"b0c0",
         X"de71",
         X"f5f2",
         X"7b4d",
         X"a80a",
         X"5b04",
         X"250f",
         X"e053",
         X"b0e0",
         X"eda2",
         X"fb66",
         X"9ee0",
         X"16ea",
         X"b9e7",
         X"e276",
         X"153f",
         X"c076",
         X"ca5a",
         X"1dfa",
         X"c3e9",
         X"530a",
         X"7b90",
         X"3af2",
         X"d808",
         X"1586",
         X"9b97",
         X"6804",
         X"6131",
         X"5159",
         X"c2bf",
         X"1828",
         X"85c3",
         X"ecde",
         X"d00e",
         X"a957",
         X"adc4",
         X"ec42",
         X"981f",
         X"1939",
         X"f88e",
         X"322a",
         X"332c",
         X"2377",
         X"889a",
         X"919d",
         X"d1d1",
         X"07e7",
         X"278a",
         X"e85a",
         X"51ea",
         X"37e6",
         X"77f9",
         X"0805",
         X"d33d",
         X"9b0b",
         X"1c68",
         X"4760",
         X"ec67",
         X"9740",
         X"c437",
         X"a756",
         X"c7f0",
         X"f0a9",
         X"06f3",
         X"f51a",
         X"6018",
         X"049f",
         X"7191",
         X"2341",
         X"752b",
         X"bd9f",
         X"2d16",
         X"ca00",
         X"2bfc",
         X"31f5",
         X"9168",
         X"c13a",
         X"b95b",
         X"2af7",
         X"582a",
         X"d82f",
         X"47d0",
         X"2ce3",
         X"81b8",
         X"1a28",
         X"e303",
         X"6a7e",
         X"804b",
         X"f663",
         X"eca2",
         X"f5ff",
         X"4529",
         X"d28e",
         X"48e5",
         X"f8ab",
         X"8928",
         X"ad06",
         X"38c5",
         X"f936",
         X"a719",
         X"d754",
         X"1b7e",
         X"b116",
         X"3d0a",
         X"be2b",
         X"6adc",
         X"c4e1",
         X"43f8",
         X"b04c",
         X"c999",
         X"9753",
         X"1382",
         X"2ef1",
         X"0bee",
         X"ecfd",
         X"24fa",
         X"25ca",
         X"2dd4",
         X"22c3",
         X"71f7",
         X"b57d",
         X"3593",
         X"bc7a",
         X"9e91",
         X"1b78",
         X"f33c",
         X"f56f",
         X"2975",
         X"4531",
         X"d9a8",
         X"2979",
         X"ea89",
         X"5789",
         X"553a",
         X"c1a6",
         X"1d0c",
         X"150e",
         X"0a6e",
         X"5eb4",
         X"c3c9",
         X"d16d",
         X"9e85",
         X"7099",
         X"2543",
         X"fabe",
         X"23aa",
         X"0b73",
         X"25b6",
         X"0b3c",
         X"3897",
         X"05c2",
         X"7e63",
         X"b7fb",
         X"9b16",
         X"9c15",
         X"9048",
         X"e793",
         X"f2c9",
         X"dda6",
         X"4375",
         X"20be",
         X"45a1",
         X"6ecf",
         X"7906",
         X"b129",
         X"a38d",
         X"323f",
         X"9805",
         X"0681",
         X"07c5",
         X"5c74",
         X"fc1f",
         X"e4ba",
         X"2be3",
         X"3dc3",
         X"0522",
         X"d904",
         X"a666",
         X"160a",
         X"c31c",
         X"8b61",
         X"4143",
         X"be27",
         X"f141",
         X"3013",
         X"dbf6",
         X"3c81",
         X"e50c",
         X"2ef4",
         X"343c",
         X"f13b",
         X"8503",
         X"d4b3",
         X"eca0",
         X"c530",
         X"b272",
         X"525c",
         X"ee0f",
         X"6345",
         X"e425",
         X"44e5",
         X"e594",
         X"4efb",
         X"414d",
         X"e09d",
         X"b74d",
         X"4a58",
         X"7306",
         X"d3db",
         X"2bd8",
         X"f04b",
         X"5560",
         X"44d4",
         X"aad1",
         X"5cab",
         X"7d68",
         X"03b1",
         X"6260",
         X"1689",
         X"a79e",
         X"b32a",
         X"e82e",
         X"3fab",
         X"5359",
         X"4a3b",
         X"d18b",
         X"08b8",
         X"9707",
         X"9c99",
         X"a2e4",
         X"2dbc",
         X"fec4",
         X"b091",
         X"feb9",
         X"a5ca",
         X"8e13",
         X"59c8",
         X"0f81",
         X"6dfb",
         X"3259",
         X"1532",
         X"50be",
         X"6107",
         X"7d29",
         X"8022",
         X"5d8d",
         X"1cd1",
         X"7d6d",
         X"0716",
         X"fac2",
         X"4d25",
         X"ba54",
         X"ff83",
         X"669e",
         X"131d",
         X"585e",
         X"3d18",
         X"1603",
         X"bf2a",
         X"2a9a",
         X"955f",
         X"203f",
         X"2934",
         X"3ad1",
         X"6408",
         X"7b78",
         X"44df",
         X"14da",
         X"6da6",
         X"1481",
         X"8459",
         X"9ef1",
         X"5cdb",
         X"fbfb",
         X"5848",
         X"b59c",
         X"0d63",
         X"2140",
         X"8831",
         X"1d5e",
         X"dcc7",
         X"8cae",
         X"fd54",
         X"b148",
         X"9f82",
         X"b49b",
         X"a582",
         X"b067",
         X"8aeb",
         X"229c",
         X"c828",
         X"09e1",
         X"31f6",
         X"ffc6",
         X"092a",
         X"f1f8",
         X"9fba",
         X"fd88",
         X"5a5e",
         X"5fba",
         X"c532",
         X"b55e",
         X"10a3",
         X"23eb",
         X"eac2",
         X"b4bb",
         X"72ac",
         X"9503",
         X"9b10",
         X"a45d",
         X"aa9d",
         X"1ef7",
         X"12df",
         X"8d55",
         X"6e63",
         X"3a8a",
         X"3ce3",
         X"903b",
         X"5c46",
         X"6f35",
         X"7c02",
         X"5be3",
         X"491a",
         X"036d",
         X"ad77",
         X"e60a",
         X"a249",
         X"87e8",
         X"706c",
         X"cd22",
         X"cba8",
         X"d53b",
         X"f792",
         X"25f0",
         X"8675",
         X"579b",
         X"0f1d",
         X"5aa6",
         X"d90f",
         X"f22f",
         X"8de3",
         X"ad57",
         X"29ae",
         X"d4b1",
         X"6603",
         X"9e3f",
         X"7d09",
         X"0a3c",
         X"34f9",
         X"7fdf",
         X"c9b0",
         X"ea1e",
         X"f700",
         X"4393",
         X"5176",
         X"99a8",
         X"ad99",
         X"dc11",
         X"8e84",
         X"059a",
         X"d5fa",
         X"acf9",
         X"b57e",
         X"67b8",
         X"2ce6",
         X"f7ee",
         X"6982",
         X"9aa1",
         X"3edc",
         X"3c7c",
         X"0fd6",
         X"af27",
         X"18e3",
         X"ccc9",
         X"a256",
         X"b66d",
         X"651b",
         X"924b",
         X"be14",
         X"8dc3",
         X"f115",
         X"8367",
         X"65ae",
         X"b258",
         X"97e7",
         X"ceb0",
         X"f4c0",
         X"9a07",
         X"7ed2",
         X"d504",
         X"cc1f",
         X"8fe2",
         X"cc5a",
         X"8bde",
         X"0164",
         X"42ed",
         X"218e",
         X"9703",
         X"94b3",
         X"46f9",
         X"67b7",
         X"08a5",
         X"9bf2",
         X"5368",
         X"d68e",
         X"cb42",
         X"3f0e",
         X"82a5",
         X"8c67",
         X"2afd",
         X"1a7d",
         X"06af",
         X"3ace",
         X"350f",
         X"4808",
         X"c9b9",
         X"314a",
         X"0e82",
         X"e582",
         X"8fc4",
         X"47ba",
         X"d66c",
         X"1b9d",
         X"3dc3",
         X"9ad5",
         X"a0bd",
         X"0caf",
         X"fc38",
         X"63f6",
         X"4c89",
         X"3bfe",
         X"8d24",
         X"92a9",
         X"96aa",
         X"4c61",
         X"7169",
         X"2f08",
         X"a1d0",
         X"3904",
         X"9c40",
         X"9e14",
         X"2406",
         X"d42d",
         X"2760",
         X"3fc7",
         X"154c",
         X"3d73",
         X"bc1e",
         X"3c26",
         X"7879",
         X"5def",
         X"9613",
         X"ddcf",
         X"de84",
         X"2f5e",
         X"1913",
         X"4a14",
         X"de1f",
         X"b4be",
         X"9630",
         X"459d",
         X"b4a7",
         X"e366",
         X"0d41",
         X"ba9d",
         X"2456",
         X"fc07",
         X"a6df",
         X"482d",
         X"99c1",
         X"cb48",
         X"bcc4",
         X"07e7",
         X"976c",
         X"e7c3",
         X"9ad7",
         X"9cbf",
         X"48d0",
         X"caa4",
         X"1a81",
         X"76e4",
         X"eeb7",
         X"31db",
         X"4213",
         X"eec2",
         X"27cf",
         X"9c19",
         X"6f0b",
         X"affd",
         X"c424",
         X"4c3f",
         X"fcd4",
         X"44da",
         X"e561",
         X"c5df",
         X"8988",
         X"2c5d",
         X"edf8",
         X"f3a5",
         X"1c20",
         X"8f35",
         X"d0d9",
         X"45d1",
         X"3249",
         X"a016",
         X"a152",
         X"97a4",
         X"8201",
         X"ec51",
         X"27d4",
         X"3911",
         X"07fd",
         X"9bdb",
         X"21bb",
         X"a062",
         X"a262",
         X"993d",
         X"a45c",
         X"ab13",
         X"b23d",
         X"d146",
         X"d101",
         X"b7b2",
         X"c044",
         X"6497",
         X"3406",
         X"0e45",
         X"af37",
         X"b648",
         X"93cd",
         X"69ef",
         X"34eb",
         X"0ecb",
         X"d03d",
         X"aa8c",
         X"1f5c",
         X"7ce9",
         X"aba1",
         X"c1ff",
         X"e595",
         X"92f1",
         X"2f21",
         X"e703",
         X"7b9b",
         X"e6f6",
         X"1ee4",
         X"a785",
         X"e1a0",
         X"a940",
         X"4214",
         X"5f01",
         X"d9cd",
         X"2f7f",
         X"cb4d",
         X"07d7",
         X"551a",
         X"18f5",
         X"d5d7",
         X"cc9a",
         X"f3dd",
         X"ec33",
         X"dc0f",
         X"0eee",
         X"3e17",
         X"eca1",
         X"edea",
         X"9ff8",
         X"8641",
         X"ca4a",
         X"d149",
         X"2758",
         X"74fa",
         X"6f8c",
         X"f538",
         X"bd90",
         X"438f",
         X"4263",
         X"3d9b",
         X"3e62",
         X"9b1e",
         X"2e7b",
         X"f698",
         X"b650",
         X"9939",
         X"52d6",
         X"accd",
         X"a9e0",
         X"2a7e",
         X"64f7",
         X"043d",
         X"33e4",
         X"a752",
         X"7416",
         X"0a77",
         X"2e03",
         X"895c",
         X"4f28",
         X"3fa5",
         X"9ec5",
         X"0669",
         X"d36a",
         X"0be4",
         X"e61d",
         X"ea44",
         X"ae45",
         X"c161",
         X"8542",
         X"6c76",
         X"2759",
         X"6ec0",
         X"a9dc",
         X"6bcd",
         X"4b6f",
         X"13d0",
         X"f0a6",
         X"c1f3",
         X"4080",
         X"ba8a",
         X"906f",
         X"4470",
         X"2bd4",
         X"3718",
         X"245e",
         X"eb47",
         X"e409",
         X"50ef",
         X"d143",
         X"5086",
         X"4a01",
         X"5a2e",
         X"0171",
         X"22bb",
         X"736e",
         X"f1a8",
         X"8f5d",
         X"5de3",
         X"2196",
         X"dae6",
         X"7f3c",
         X"b963",
         X"2707",
         X"1ae1",
         X"e323",
         X"a466",
         X"866f",
         X"ebce",
         X"af21",
         X"39cc",
         X"ded0",
         X"5770",
         X"3bf6",
         X"122f",
         X"ad46",
         X"7517",
         X"c3ec",
         X"6cb1",
         X"b949",
         X"dfa2",
         X"9666",
         X"23df",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000",
         X"0000");

  CONSTANT filter_out_expected : filter_out_type :=
    (
         SLICE(X"0000000",26),
         SLICE(X"0e87e2f",26),
         SLICE(X"1ff7c01",26),
         SLICE(X"1177dd1",26),
         SLICE(X"0007fff",26),
         SLICE(X"32a77f6",26),
         SLICE(X"21e8860",26),
         SLICE(X"23d0f60",26),
         SLICE(X"25d1760",26),
         SLICE(X"27d1f60",26),
         SLICE(X"29d2760",26),
         SLICE(X"2bd2f60",26),
         SLICE(X"2dd3760",26),
         SLICE(X"2fd3f60",26),
         SLICE(X"31d4760",26),
         SLICE(X"33d4f60",26),
         SLICE(X"35d5760",26),
         SLICE(X"37d5f60",26),
         SLICE(X"39d6760",26),
         SLICE(X"3bd6f60",26),
         SLICE(X"3dd7760",26),
         SLICE(X"3fd7f60",26),
         SLICE(X"01d8760",26),
         SLICE(X"03d8f60",26),
         SLICE(X"05d9760",26),
         SLICE(X"07d9f60",26),
         SLICE(X"09da760",26),
         SLICE(X"0bdaf60",26),
         SLICE(X"0ddb760",26),
         SLICE(X"0fdbf60",26),
         SLICE(X"11dc760",26),
         SLICE(X"13dcf60",26),
         SLICE(X"15dd760",26),
         SLICE(X"17ddf60",26),
         SLICE(X"19de760",26),
         SLICE(X"1bdef60",26),
         SLICE(X"1ddf760",26),
         SLICE(X"1201a8c",26),
         SLICE(X"0017fbe",26),
         SLICE(X"0c24698",26),
         SLICE(X"068b5df",26),
         SLICE(X"3df7217",26),
         SLICE(X"0083180",26),
         SLICE(X"3f5256d",26),
         SLICE(X"00542b4",26),
         SLICE(X"3fce497",26),
         SLICE(X"3ff4b7c",26),
         SLICE(X"001c954",26),
         SLICE(X"3feab1b",26),
         SLICE(X"000ce91",26),
         SLICE(X"000dc73",26),
         SLICE(X"3fef8ad",26),
         SLICE(X"3ffdd65",26),
         SLICE(X"0006491",26),
         SLICE(X"3ffcab3",26),
         SLICE(X"0001322",26),
         SLICE(X"0004abf",26),
         SLICE(X"3ffbe9b",26),
         SLICE(X"3ffbadd",26),
         SLICE(X"000481b",26),
         SLICE(X"00033bf",26),
         SLICE(X"3ffc21b",26),
         SLICE(X"3ffeb26",26),
         SLICE(X"0002689",26),
         SLICE(X"3ffe7a2",26),
         SLICE(X"00002bf",26),
         SLICE(X"000431a",26),
         SLICE(X"3ffc8c7",26),
         SLICE(X"3ffc6f6",26),
         SLICE(X"0004a78",26),
         SLICE(X"3ffe3bc",26),
         SLICE(X"3fb3853",26),
         SLICE(X"3ff5759",26),
         SLICE(X"02fd501",26),
         SLICE(X"0318fde",26),
         SLICE(X"007f33c",26),
         SLICE(X"3eb13b8",26),
         SLICE(X"3e2f1ea",26),
         SLICE(X"3bc3686",26),
         SLICE(X"001e15c",26),
         SLICE(X"3de1e5f",26),
         SLICE(X"3feb3ae",26),
         SLICE(X"01eaa48",26),
         SLICE(X"3ec93af",26),
         SLICE(X"000e07a",26),
         SLICE(X"3e4d16b",26),
         SLICE(X"3e70032",26),
         SLICE(X"3f26d24",26),
         SLICE(X"3f720a6",26),
         SLICE(X"01f0343",26),
         SLICE(X"3f1051b",26),
         SLICE(X"028d6ee",26),
         SLICE(X"045643e",26),
         SLICE(X"00718dc",26),
         SLICE(X"3ce8a67",26),
         SLICE(X"3f7245a",26),
         SLICE(X"3bb1edd",26),
         SLICE(X"3b931ee",26),
         SLICE(X"3fb99dc",26),
         SLICE(X"3d1a64d",26),
         SLICE(X"3adf8ac",26),
         SLICE(X"3a18a92",26),
         SLICE(X"3f03178",26),
         SLICE(X"0027962",26),
         SLICE(X"02c4728",26),
         SLICE(X"3fd8a66",26),
         SLICE(X"3fecb3b",26));

END filter_tb_data;
-- -------------------------------------------------------------
--
-- Module: filter_tb
-- Generated by MATLAB(R) 9.13 and Filter Design HDL Coder 3.1.12.
-- Generated on: 2023-03-21 12:26:45
-- -------------------------------------------------------------

-- -------------------------------------------------------------
LIBRARY IEEE;
USE IEEE.std_logic_1164.all;
USE IEEE.numeric_std.ALL;

USE work.filter_tb_pkg.ALL;

USE work.filter_tb_data.ALL;

ENTITY filter_tb IS

END filter_tb;


ARCHITECTURE rtl OF filter_tb IS
  -- -------------------------------------------------------------
  -- Component Declarations
  -- -------------------------------------------------------------
  COMPONENT CIC32
   PORT( clk                             :   IN    std_logic; 
         clk_enable                      :   IN    std_logic; 
         reset                           :   IN    std_logic; 
         filter_in                       :   IN    std_logic_vector(15 DOWNTO 0); -- sfix16_En15
         filter_out                      :   OUT   std_logic_vector(25 DOWNTO 0); -- sfix26_En15
         ce_out                          :   OUT   std_logic  
         );
  END COMPONENT;

  -- -------------------------------------------------------------
  -- Component Configuration Statements
  -- -------------------------------------------------------------
  FOR ALL : CIC32
    USE ENTITY work.CIC32(rtl);

  -- Constants
  CONSTANT clk_high                         : time := 5 ns;
  CONSTANT clk_low                          : time := 5 ns;
  CONSTANT clk_period                       : time := 10 ns;
  CONSTANT clk_hold                         : time := 2 ns;
  CONSTANT MAX_ERROR_COUNT                : integer := 3424; -- uint32


  -- Signals
  SIGNAL clk                              : std_logic; -- boolean
  SIGNAL clk_enable                       : std_logic; -- boolean
  SIGNAL reset                            : std_logic; -- boolean
  SIGNAL filter_in                        : std_logic_vector(15 DOWNTO 0); -- sfix16_En15
  SIGNAL filter_out                       : std_logic_vector(25 DOWNTO 0); -- sfix26_En15
  SIGNAL ce_out                           : std_logic; -- boolean

  SIGNAL tb_enb                           : std_logic; -- boolean
  SIGNAL srcDone                          : std_logic; -- boolean
  SIGNAL snkDone                          : std_logic; -- boolean
  SIGNAL testFailure                      : std_logic; -- boolean
  SIGNAL tbenb_dly                        : std_logic; -- boolean
  SIGNAL rdEnb                            : std_logic; -- boolean
  SIGNAL filter_in_data_log_rdenb         : std_logic; -- boolean
  SIGNAL filter_in_data_log_addr          : unsigned(11 DOWNTO 0); -- ufix12
  SIGNAL filter_in_data_log_done          : std_logic; -- boolean
  SIGNAL filter_out_testFailure           : std_logic; -- boolean
  SIGNAL filter_out_errCnt                : integer; -- uint32
  SIGNAL filter_out_rdenb                 : std_logic; -- boolean
  SIGNAL filter_out_addr                  : unsigned(6 DOWNTO 0); -- ufix7
  SIGNAL filter_out_done                  : std_logic; -- boolean
  SIGNAL filter_out_ref                   : std_logic_vector(25 DOWNTO 0); -- sfix26_En15
  SIGNAL filter_out_dataTable             : std_logic_vector(25 DOWNTO 0); -- sfix26_En15
  SIGNAL filter_out_refTmp                : std_logic_vector(25 DOWNTO 0); -- sfix26_En15
  SIGNAL regout                           : std_logic_vector(25 DOWNTO 0); -- sfix26_En15
  SIGNAL check1_Done                      : std_logic; -- boolean


BEGIN
  -- Component Instances
  u_CIC32: CIC32
    PORT MAP (
              clk                              => clk,
              clk_enable                       => clk_enable,
              reset                            => reset,
              filter_in                        => filter_in,
              filter_out                       => filter_out,
              ce_out                           => ce_out      );


  -- Block Statements
  -- -------------------------------------------------------------
  -- Driving the test bench enable
  -- -------------------------------------------------------------

  tb_enb <= '0' WHEN reset = '1' ELSE 
            '1' WHEN snkDone = '0' ELSE 
            '0' AFTER clk_period * 2;

  completed_msg: PROCESS (clk, reset)
  BEGIN
    IF (reset = '1') THEN 
       -- Nothing to reset here.
    ELSIF clk'event AND clk = '1' THEN
      IF snkDone='1' THEN
        IF (testFailure = '0') THEN
              ASSERT FALSE
                REPORT "**************TEST COMPLETED (PASSED)**************"
                SEVERITY NOTE;
        ELSE
              ASSERT FALSE
                REPORT "**************TEST COMPLETED (FAILED)**************"
                SEVERITY NOTE;
        END IF;
      END IF;
    END IF;
  END PROCESS completed_msg;

  -- -------------------------------------------------------------
  -- System Clock (fast clock) and reset
  -- -------------------------------------------------------------

  clk_gen: PROCESS
  BEGIN
    clk <= '1';
    WAIT FOR clk_high;
    clk <= '0';
    WAIT FOR clk_low;
    IF snkDone = '1' THEN
      clk <= '1';
      WAIT FOR clk_high;
      clk <= '0';
      WAIT FOR clk_low;
      WAIT;
    END IF;
  END PROCESS clk_gen;

  reset_gen: PROCESS
  BEGIN
    reset <= '1';
    WAIT FOR clk_period * 2;
    WAIT UNTIL clk'event AND clk = '1';
    WAIT FOR clk_hold;
    reset <= '0';
    WAIT;
  END PROCESS reset_gen;

  -- -------------------------------------------------------------
  -- Testbench clock enable
  -- -------------------------------------------------------------

  tb_enb_delay : PROCESS (clk, reset)
  BEGIN
    IF reset = '1' THEN
      tbenb_dly <= '0';
    ELSIF clk'event AND clk = '1' THEN
      IF tb_enb = '1' THEN
        tbenb_dly <= tb_enb;
      END IF;
    END IF; 
  END PROCESS tb_enb_delay;

  rdEnb <= tbenb_dly WHEN snkDone =  '0' ELSE
           '0';

  -- -------------------------------------------------------------
  -- Read the data and transmit it to the DUT
  -- -------------------------------------------------------------

  filter_in_data_log_procedure (
    clk       => clk,
    reset     => reset,
    rdenb     => filter_in_data_log_rdenb,
    addr      => filter_in_data_log_addr,
    done      => filter_in_data_log_done);

  filter_in_data_log_rdenb <= rdEnb;

  stimuli_filter_in_data_log : PROCESS(filter_in_data_log_addr, filter_in_data_log_rdenb)
  BEGIN
    IF filter_in_data_log_rdenb = '1' THEN
      filter_in <= filter_in_data_log_force(TO_INTEGER(filter_in_data_log_addr)) AFTER clk_hold;
    END IF;
  END PROCESS stimuli_filter_in_data_log;

  -- -------------------------------------------------------------
  -- Create done signal for Input data
  -- -------------------------------------------------------------

  srcDone <= filter_in_data_log_done;

  -- -------------------------------------------------------------
  --  Checker: Checking the data received from the DUT.
  -- -------------------------------------------------------------

  filter_out_procedure (
    clk       => clk,
    reset     => reset,
    rdenb     => filter_out_rdenb,
    addr      => filter_out_addr,
    done      => filter_out_done);

  filter_out_rdenb <= ce_out;

filter_out_dataTable <= filter_out_expected(TO_INTEGER(filter_out_addr)) AFTER clk_hold;

-- ---- Bypass Register ----
  DataHoldRegister_temp_process2 : PROCESS (clk, reset)
  BEGIN
    IF reset = '1' THEN
      regout <= (OTHERS => '0');
    ELSIF clk'event AND clk = '1' THEN
      IF ce_out = '1' THEN
        regout <= (filter_out_dataTable);
      END IF;
    END IF;
  END PROCESS DataHoldRegister_temp_process2;

  filter_out_refTmp <= (filter_out_dataTable) WHEN ( ce_out = '1' ) ELSE
                       (regout);


  filter_out_ref <= std_logic_vector(filter_out_refTmp);

  checker_1: PROCESS(clk, reset)
  BEGIN
    IF reset = '1' THEN
      filter_out_errCnt <= 0;
      filter_out_testFailure <= '0';
    ELSIF clk'event and clk ='1' THEN
      IF filter_out_rdenb = '1' THEN
        IF (Is_X(filter_out) OR abs(signed(filter_out)  - signed(filter_out_expected(TO_INTEGER(filter_out_addr)))) > 15) THEN
          filter_out_errCnt <= filter_out_errCnt + 1;
          filter_out_testFailure <= '1';
          ASSERT FALSE 
            REPORT "Error in filter_out: Expected " 
            & to_hex(filter_out_expected(TO_INTEGER(filter_out_addr)))
            & " Actual "
            & to_hex(filter_out)
            SEVERITY ERROR;
          IF filter_out_errCnt >= MAX_ERROR_COUNT THEN
            ASSERT FALSE
              REPORT "Number of errors have exceeded the maximum error"
              SEVERITY Warning;
          END IF;
        END IF;
      END IF;
    END IF;
  END PROCESS checker_1;

  checkDone_1: PROCESS(clk, reset)
  BEGIN
    IF reset = '1' THEN
      check1_Done <= '0';
    ELSIF clk'event and clk ='1' THEN
      IF check1_Done = '0' AND filter_out_done = '1' AND filter_out_rdenb = '1' THEN
        check1_Done <= '1';
      END IF;
    END IF;
  END PROCESS checkDone_1;
  -- -------------------------------------------------------------
  -- Create done and test failure signal for output data
  -- -------------------------------------------------------------

  snkDone <= check1_Done;

  testFailure <= filter_out_testFailure;

  -- -------------------------------------------------------------
  -- Global clock enable
  -- -------------------------------------------------------------
  clk_enable <= tbenb_dly AFTER clk_hold WHEN snkDone = '0' ELSE
                '0' AFTER clk_hold;

  -- Assignment Statements



END rtl;
